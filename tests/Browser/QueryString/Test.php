...
class Test extends TestCase
{
    public function test_core_query_string_pushstate_logic()
    {
        $this->browse(function (Browser $browser) {
            Actcmscss::visit($browser, Component::class, '?foo=baz&eoo=lob')
                /*
                ...
                /*
                 * Check that Actcmscss doesn't mess with query string order.
                 */
                 ...
                 */
                ->waitForActcmscss()->type('@input', 'bob')
                ...
                ->waitForActcmscss()->type('@bar-input', 'except-value')
                ...
                ->waitForActcmscss()->click('@show-nested')
                ->pause(25)
                ->assertQueryStringHas('baz', 'bop')
                ->assertSeeIn('@baz-output', 'bop')
                ->waitForActcmscss()->type('@baz-input', 'lop')
                ->assertQueryStringHas('baz', 'lop')
                ->waitForActcmscss()->type('@input', 'plop')
                ->waitForActcmscss()->type('@baz-input', 'ploop')
                ...
                ->waitForActcmscss()->click('@bob.modify')
    ...
    public function test_query_string_format_in_rfc_3986()
    {
        $this->browse(function (Browser $browser) {
            Actcmscss::visit($browser, Component::class)
                ->waitForActcmscss()->type('@input', 'foo bar')
    ...
    public function test_back_button_after_refresh_works_with_nested_components()
    {
        $this->browse(function (Browser $browser) {
            Actcmscss::visit($browser, Component::class)
                ->waitForActcmscss()->click('@show-nested')
                ->waitForActcmscss()->type('@baz-input', 'foo')
                ...
                ->waitForActcmscss()->type('@baz-input', 'bar')
    ...
    {
        $this->browse(function (Browser $browser) {
            Actcmscss::visit($browser, ComponentWithExcepts::class)
                ->assertSeeIn('@output', 'On page 1');

            $this->assertStringNotContainsString('?', $browser->driver->getCurrentURL());

            Actcmscss::visit($browser, ComponentWithExcepts::class, '?page=1')
                ->assertSeeIn('@output', 'On page 1');

            $this->assertStringNotContainsString('?', $browser->driver->getCurrentURL());

            Actcmscss::visit($browser, ComponentWithExcepts::class, '?page=2')
    ...
    {
        $this->browse(function (Browser $browser) {
            Actcmscss::visit($browser, DirtyDataComponent::class)
                ->waitForActcmscss()->type('@input', 'foo')
    ...
    {
        $this->browse(function (Browser $browser) {
            Actcmscss::visit($browser, HugeComponent::class)
                ->assertSeeIn('@count', '0')
                ->waitForActcmscss()->click('@increment')
                ->waitForActcmscss()->click('@increment')
                ->waitForActcmscss()->click('@increment')
                ->waitForActcmscss()->click('@increment')
                ->waitForActcmscss()->click('@increment')
    ...
    {
        $this->browse(function (Browser $browser) {
            Actcmscss::visit($browser, ComponentWithMethodInsteadOfProperty::class)
