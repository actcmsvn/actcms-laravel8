...
export default function () {
    store.registerHook('element.initialized', (el, component) => {
        ...
        el.__actcmscss_polling_interval = intervalId
    })

    store.registerHook('element.updating', (from, to, component) => {
        if (from.__actcmscss_polling_interval !== undefined) return
        ...
        if (wireDirectives(from).missing('poll') && wireDirectives(to).has('poll')) {
            setTimeout(() => {
                ...
                from.__actcmscss_polling_interval = intervalId
            }, 0)
        }
    })
}
...
function fireActionOnInterval(node, component) {
    ...
    return setInterval(() => {
        ...
        if (store.actcmscssIsInBackground && ! directive.modifiers.includes('keep-alive')) {
        ...
        // Don't poll if actcmscss is offline as well.
        if (store.actcmscssIsOffline) return
