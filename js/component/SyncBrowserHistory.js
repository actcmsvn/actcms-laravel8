...
export default function () {
    ...
    ActcmscssStateManager.clearState()
    ...
    store.registerHook('component.initialized', component => {
        ...
        setTimeout(() => {
            ...
            ActcmscssStateManager.replaceState(url, response, component)
    ...
    store.registerHook('message.processed', (message, component) => {
        ...
        if ('path' in effects && effects.path !== window.location.href) {
            ...
            ActcmscssStateManager.pushState(url, response, component)
            ...
            if (componentIdsThatAreWritingToHistoryState.has(component.id)) {
                ActcmscssStateManager.replaceState(window.location.href, response, component)
            }
        }
    })

    window.addEventListener('popstate', event => {
        if (ActcmscssStateManager.missingState(event)) return

        ActcmscssStateManager.replayResponses(event, (response, component) => {
    ...
    function normalizeResponse(response, component) {
        ...
        // Sometimes Actcmscss doesn't return html from the server to save on bandwidth.
...
let ActcmscssStateManager = {
    ...
    updateState(method, url, response, component) {
        ...
        let fullstateObject = Object.assign(history.state || {}, { actcmscss: stateArray })
    ...
    replayResponses(event, callback) {
        if (! event.state.actcmscss) return

        let state = typeof event.state.actcmscss === 'string'
            ? new ActcmscssState(this.getFromSession(event.state.actcmscss))
            : new ActcmscssState(event.state.actcmscss)

        state.replayResponses(callback)
    },

    currentState() {
        if (! history.state) return new ActcmscssState
        if (! history.state.actcmscss) return new ActcmscssState

        let state = typeof history.state.actcmscss === 'string'
            ? new ActcmscssState(this.getFromSession(history.state.actcmscss))
            : new ActcmscssState(history.state.actcmscss)

        return state
    },

    missingState(event) {
        return ! (event.state && event.state.actcmscss)
    },

    clearState() {
        // This is to prevent exponentially increasing the size of our state on page refresh.
        if (window.history.state) window.history.state.actcmscss = (new ActcmscssState).toStateArray();
    },

    storeInSession(value) {
        let key = 'actcmscss:'+(new Date).getTime()
    ...
    tryToStoreInSession(key, value) {
        ...
        try {
            ...
            let oldestTimestamp = Object.keys(sessionStorage)
                .map(key => Number(key.replace('actcmscss:', '')))
                .sort()
                .shift()

            if (! oldestTimestamp) return

            sessionStorage.removeItem('actcmscss:'+oldestTimestamp)
...
class ActcmscssState
{
    ...
    findComponentBySignature(signature) {
        ...
        return sameNamedComponents[componentIndex] || sameNamedComponents[0] || console.warn(`actcmscss: couldn't find component on page: ${componentName}`)
    }
