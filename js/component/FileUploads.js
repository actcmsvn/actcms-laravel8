...
export default function () {
    store.registerHook('interceptWireModelAttachListener', (directive, el, component) => {
        ...
        let start = () => el.dispatchEvent(new CustomEvent('actcmscss-upload-start', { bubbles: true }))
        let finish = () => el.dispatchEvent(new CustomEvent('actcmscss-upload-finish', { bubbles: true }))
        let error = () => el.dispatchEvent(new CustomEvent('actcmscss-upload-error', { bubbles: true }))
        let progress = (progressEvent) => {
            var percentCompleted = Math.round( (progressEvent.loaded * 100) / progressEvent.total )

            el.dispatchEvent(
                new CustomEvent('actcmscss-upload-progress', {
