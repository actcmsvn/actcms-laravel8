...
export default function () {
    window.addEventListener('actcmscss:load', () => {
        if (! window.Alpine) return

        refreshAlpineAfterEveryActcmscssRequest()
...
function refreshAlpineAfterEveryActcmscssRequest() {
    if (! window.Alpine.onComponentInitialized) return

    window.Alpine.onComponentInitialized(component => {
        let actcmscssEl = component.$el.closest('[wire\\:id]')

        if (actcmscssEl && actcmscssEl.__actcmscss) {
            store.registerHook('message.processed', (message, actcmscssComponent) => {
                if (actcmscssComponent === actcmscssEl.__actcmscss) {
...
function addDollarSignWire() {
    ...
    window.Alpine.addMagicProperty('wire', function (componentEl) {
        ...
        if (!wireEl)
            console.warn(
                'Alpine: Cannot reference "$wire" outside a Actcmscss component.'
            )

        let component = wireEl.__actcmscss
            ...
            Object.entries(component.unobservedData).forEach(
                ([key, value]) => {
                    if (
                        !!value &&
                        typeof value === 'object' &&
                        value.actcmscssEntangle
                    ) {
                        // Ok, it looks like someone set an Alpine property to $wire.entangle or @entangle.
                        let actcmscssProperty = value.actcmscssEntangle
                        let isDeferred = value.isDeferred
                        let actcmscssComponent = actcmscssEl.__actcmscss

                        let actcmscssPropertyValue = actcmscssEl.__actcmscss.get(actcmscssProperty)

                        // Check to see if the Actcmscss property exists and if not log a console error
                        // and return so everything else keeps running.
                        if (typeof actcmscssPropertyValue === 'undefined') {
                            console.error(`Actcmscss Entangle Error: Actcmscss property '${actcmscssProperty}' cannot be found`)
                            return
                        }

                        // Let's set the initial value of the Alpine prop to the Actcmscss prop's value.
                        component.unobservedData[key]
                            // We need to stringify and parse it though to get a deep clone.
                            = JSON.parse(JSON.stringify(actcmscssPropertyValue))

                        let blockAlpineWatcher = false

                        // Now, we'll watch for changes to the Alpine prop, and fire the update to Actcmscss.
                        component.unobservedData.$watch(key, value => {
                            // Let's also make sure that this watcher isn't a result of a Actcmscss response.
                            // If it is, we don't need to "re-update" Actcmscss. (sending an extra useless) request.
                            if (blockAlpineWatcher === true) {
                                blockAlpineWatcher = false
                                return
                            }

                            // If the Alpine value is the same as the Actcmscss value, we'll skip the update for 2 reasons:
                            ...
                            if (
                                ...
                                JSON.stringify(
                                    actcmscssEl.__actcmscss.getPropertyValueIncludingDefers(
                                        actcmscssProperty
                                    )
                                )
                            ) return

                            // We'll tell Actcmscss to update the property, but we'll also tell Actcmscss
                            // to not call the normal property watchers on the way back to prevent another
                            // circular dependancy.
                            actcmscssComponent.set(
                                actcmscssProperty,
                                value,
                                isDeferred,
                                // Block firing of Actcmscss watchers for this data key when the request comes back.
                                // Unless it is deferred, in which cause we don't know if the state will be the same, so let it run.
                                isDeferred ? false : true
                            )
                        })

                        // We'll also listen for changes to the Actcmscss prop, and set them in Alpine.
                        actcmscssComponent.watch(
                            actcmscssProperty,
                            value => {
                                // Ensure data is deep cloned otherwise Alpine mutates Actcmscss data
...
export function alpinifyElementsForMorphdom(from, to) {
    ...
    if (from.__x) {
        ...
        // This should simulate backend Actcmscss being aware of Alpine changes.
