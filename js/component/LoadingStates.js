...
function processLoadingDirective(component, el, directive) {
    ...
    el.__actcmscss_on_finish_loading = []
    ...
    if (directives.get('target')) {
        ...
        const nonActionOrModelActcmscssDirectives = [
            'init',
            'dirty',
            'offline',
            'target',
        ...
        actionNames = directives
            .all()
            .filter(i => !nonActionOrModelActcmscssDirectives.includes(i.type))
            .map(i => i.method)
...
function doAndSetCallbackOnElToUndo(el, directive, doCallback, undoCallback) {
    ...
    if (directive.modifiers.includes('delay')) {
        let timeout = setTimeout(() => {
            doCallback()
            el.__actcmscss_on_finish_loading.push(() => undoCallback())
        }, 200)

        el.__actcmscss_on_finish_loading.push(() => clearTimeout(timeout))
    } else {
        doCallback()
        el.__actcmscss_on_finish_loading.push(() => undoCallback())
    }
...
function endLoading(els) {
    els.forEach(({ el }) => {
        while (el.__actcmscss_on_finish_loading.length > 0) {
            el.__actcmscss_on_finish_loading.shift()()
        }
    })
}
