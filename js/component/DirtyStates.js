...
export default function () {
    ...
    store.registerHook('message.received', (message, component) => {
        component.dirtyEls.forEach(element => {
            if (element.__actcmscss_dirty_cleanup) {
                element.__actcmscss_dirty_cleanup()
                delete element.__actcmscss_dirty_cleanup
            }
        })
    })
...
function setDirtyState(el, isDirty) {
    ...
    if (directive.modifiers.includes('class')) {
        ...
        if (directive.modifiers.includes('remove') !== isDirty) {
            ...
            el.__actcmscss_dirty_cleanup = () => el.classList.remove(...classes)
        } else {
            el.classList.remove(...classes)
            el.__actcmscss_dirty_cleanup = () => el.classList.add(...classes)
        }
    } else if (directive.modifiers.includes('attr')) {
        if (directive.modifiers.includes('remove') !== isDirty) {
            el.setAttribute(directive.value, true)
            el.__actcmscss_dirty_cleanup = () =>
