...
export default class Component {
    ...
    updateServerMemoFromResponseAndMergeBackIntoResponse(message) {
        ...
        // because browsers mess with the object key order which will break Actcmscss request checksum checks.
        ...
        Object.entries(message.response.serverMemo).forEach(([key, value]) => {
            ...
            if (key === 'data') {
                Object.entries(value || {}).forEach(([dataKey, dataValue]) => {
                    ...
                    // Because Actcmscss (for payload reduction purposes) only returns the data that has changed,
    ...
    handleMorph(dom) {
        ...
        morphdom(this.el, dom, {
            ...
            onNodeDiscarded: node => {
                ...
                if (node.__actcmscss) {
                    store.removeComponent(node.__actcmscss)
                }
            ...
            onBeforeElUpdated: (from, to) => {
                ...
                // Honor the "wire:ignore" attribute or the .__actcmscss_ignore element property.
                if (
                    fromDirectives.has('ignore') ||
                    from.__actcmscss_ignore === true ||
                    from.__actcmscss_ignore_self === true
                ) {
                    if (
                        (fromDirectives.has('ignore') &&
                            fromDirectives
                ...
                // Give the root Actcmscss "to" element, the same object reference as the "from"
                // element. This ensures new Alpine magics like $wire and @entangle can
                // initialize in the context of a real Actcmscss component object.
                if (DOM.isComponentRootEl(from)) to.__actcmscss = this
    ...
    get $wire() {
        ...
        return (this.dollarWireProxy = new Proxy(refObj, {
            get(object, property) {
                ...
                // Forward "emits" to base Actcmscss object.
