...
class TemporaryUploadedFile extends UploadedFile
{
    ...
    public function temporaryUrl()
    {
        ...
        return URL::temporarySignedRoute(
            'actcmscss.preview-file', now()->addMinutes(30), ['filename' => $this->getFilename()]
        );
    }

    public function isPreviewable()
    {
        $supportedPreviewTypes = config('actcmscss.temporary_file_upload.preview_mimes', [
    ...
    public static function createFromactcmscss($filePath)
    {
    ...
    public static function canUnserialize($subject)
    {
        if (is_string($subject)) {
            return (string) str($subject)->startsWith(['actcmscss-file:', 'actcmscss-files:']);
        }
    ...
    public static function unserializeFromActcmscssRequest($subject)
    {
        if (is_string($subject)) {
            if (str($subject)->startsWith('actcmscss-file:')) {
                return static::createFromactcmscss(str($subject)->after('actcmscss-file:'));
            }

            if (str($subject)->startsWith('actcmscss-files:')) {
                $paths = json_decode(str($subject)->after('actcmscss-files:'), true);

                return collect($paths)->map(function ($path) { return static::createFromActcmscss($path); })->toArray();
            }
        }

        if (is_array($subject)) {
            foreach ($subject as $key => $value) {
                $subject[$key] =  static::unserializeFromActcmscssRequest($value);
            }
        }
    ...
    public function serializeForActcmscssResponse()
    {
        return 'actcmscss-file:'.$this->getFilename();
    }

    public static function serializeMultipleForActcmscssResponse($files)
    {
        return 'actcmscss-files:'.json_encode(collect($files)->map->getFilename());
    }
}
