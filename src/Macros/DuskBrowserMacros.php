...
class DuskBrowserMacros
{
    ...
    public function waitForActcmscssToLoad()
    {
        return function () {
            /** @var \Laravel\Dusk\Browser $this */
            return $this->waitUsing(5, 75, function () {
                return $this->driver->executeScript("return !! window.Actcmscss.components.initialRenderIsFinished");
            });
        };
    }

    public function waitForActcmscss()
    {
        return function ($callback = null) {
            ...
            $this->script([
                "window.duskIsWaitingForActcmscssRequest{$id} = true",
                "window.Actcmscss.hook('message.sent', () => { window.duskIsWaitingForActcmscssRequest{$id} = true })",
                "window.Actcmscss.hook('message.processed', () => { delete window.duskIsWaitingForActcmscssRequest{$id} })",
                "window.Actcmscss.hook('message.failed', () => { delete window.duskIsWaitingForActcmscssRequest{$id} })",
            ]);

            if ($callback) {
                $callback($this);

                // Wait a quick sec for Actcmscss to hear a click and send a request.
                $this->pause(25);

                return $this->waitUsing(5, 50, function () use ($id) {
                    return $this->driver->executeScript("return window.duskIsWaitingForActcmscssRequest{$id} === undefined");
                }, 'Actcmscss request was never triggered');
            }

            // If no callback is passed, make ->waitForActcmscss a higher-order method.
                ...
                public function __call($method, $params)
                {
                    return tap($this->browser->{$method}(...$params), function ($browser) {
                        $browser->waitUsing(5, 25, function () use ($browser) {
                            return $browser->driver->executeScript("return window.duskIsWaitingForActcmscssRequest{$this->id} === undefined");
                        }, 'Actcmscss request was never triggered');
                    });
                }
            };
