...
abstract class Component
{
    ...
    public function __invoke(Container $container, Route $route)
    {
        ...
        return app('view')->file(__DIR__."/Macros/actcmscss-view-{$layoutType}.blade.php", [
            'view' => $this->initialLayoutConfiguration['view'] ?? config('actcmscss.layout'),
    ...
    protected function ensureIdPropertyIsntOverridden()
    {
        throw_if(
            ...
            new CannotUseReservedActcmscssComponentProperties('id', $this::getName())
        );
    }
    ...
    {
        $namespace = collect(explode('.', str_replace(['/', '\\'], '.', config('actcmscss.class_namespace'))))
            ->map([Str::class, 'kebab'])
            ->implode('.');
    ...
    public function renderToView()
    {
        ...
        Actcmscss::dispatch('component.rendering', $this);

        $view = method_exists($this, 'render')
            ? app()->call([$this, 'render'])
            : view("actcmscss.{$this::getName()}");
        ...
        if ($view->actcmscssLayout) {
            $this->initialLayoutConfiguration = $view->actcmscssLayout;
        }

        Actcmscss::dispatch('component.rendered', $this, $view);
    ...
    public function output($errors = null)
    {
        ...
        // with our own. However, we only want Actcmscss hijackings,
        // while we're rendering Actcmscss components. So we'll
        // activate it here, and deactivate it at the end
        // of this method.
        $engine = app('view.engine.resolver')->resolve('blade');
        $engine->startactcmscssRendering($this);
        ...
        Actcmscss::dispatch('view:render', $view);

        $engine->endActcmscssRendering();
