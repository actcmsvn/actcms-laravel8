...
namespace Actcmscss\Testing;

use Mockery;
use Actcmscss\Actcmscss;
use Illuminate\Routing\Route;
use Illuminate\Support\Facades\View;
use Actcmscss\GenerateSignedUploadUrl;
use Illuminate\Routing\RouteCollection;
use Illuminate\Support\Traits\Macroable;
use Facades\Actcmscss\GenerateSignedUploadUrl as GenerateSignedUploadUrlFacade;
use Actcmscss\Exceptions\PropertyNotFoundException;
use Actcmscss\ActcmscssManager;

use function Actcmscss\str;

/** @mixin \Illuminate\Testing\TestResponse */
class TestableActcmscss
{
    ...
    {
        Actcmscss::listen('view:render', function ($view) {
            $this->lastRenderedView = $view;
        });

        Actcmscss::listen('failed-validation', function ($validator) {
            $this->lastValidator = $validator;
        });

        Actcmscss::listen('component.hydrate.subsequent', function ($validator) {
        ...
        Actcmscss::listen('mounted', function ($response) {
        ...
        if (class_exists($name)) {
            $componentClass = $name;
            app('actcmscss')->component($name = str()->random(20), $componentClass);
        }
    ...
    {
        // Sometimes Actcmscss will skip rendering the DOM.
    ...
    {
        $randomRoutePath = '/testing-actcmscss/'.str()->random(20);
    ...
    private function registerRouteBeforeExistingRoutes($path, $closure)
    {
        ...
        $actcmscssTestingRoute = new Route(['GET', 'HEAD'], $path, $closure);
        ...
        $runningCollection->add($actcmscssTestingRoute);
    ...
    {
        $result = $this->callEndpoint('POST', '/actcmscss/message/'.$this->componentName, [
