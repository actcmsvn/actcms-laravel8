...
use Exception;
use Actcmscss\Testing\TestableActcmscss;
use Illuminate\Contracts\Auth\Authenticatable;
use Actcmscss\Exceptions\ComponentNotFoundException;

class ActcmscssManager
{
    protected $listeners = [];
    ...
    public static $isActcmscssRequestTestingOverride = false;
    ...
    {
        $finder = app(ActcmscssComponentsFinder::class);

        $class = false;

        $class = $class ?: (
            // Let's first check if the user registered the component using:
            // Actcmscss::component('name', [Actcmscss component class]);
    ...
    {
        return new TestableActcmscss($name, $params, $this->queryParamsForTesting);
    }

    public function visit($browser, $class, $queryString = '')
    {
        $url = '/actcmscss-dusk/'.urlencode($class).$queryString;

        return $browser->visit($url)->waitForActcmscssToLoad();
    }
...
<style>
    ...
    input:-webkit-autofill, select:-webkit-autofill, textarea:-webkit-autofill {
        ...
        animation-name: actcmscssautofill;
    }

    @keyframes actcmscssautofill { from {} }
</style>
HTML;
    }
    ...
    protected function javaScriptAssets($options)
    {
        ...
        $appUrl = config('actcmscss.asset_url') ?: rtrim($options['asset_url'] ?? '', '/');

        $jsactcmscssToken = app()->has('session.store') ? "'" . csrf_token() . "'" : 'null';

        $manifest = json_decode(file_get_contents(__DIR__.'/../dist/manifest.json'), true);
        $versionedFileName = $manifest['/actcmscss.js'];

        // Default to dynamic `actcmscss.js` (served by a Laravel route).
        $fullAssetPath = "{$appUrl}/actcmscss{$versionedFileName}";
        $assetWarning = null;

        $nonce = isset($options['nonce']) ? "nonce=\"{$options['nonce']}\"" : '';

        // Use static assets if they have been published
        if (file_exists(public_path('vendor/actcmscss/manifest.json'))) {
            $publishedManifest = json_decode(file_get_contents(public_path('vendor/actcmscss/manifest.json')), true);
            $versionedFileName = $publishedManifest['/actcmscss.js'];

            $fullAssetPath = ($this->isOnVapor() ? config('app.asset_url') : $appUrl).'/vendor/actcmscss'.$versionedFileName;

            if ($manifest !== $publishedManifest) {
                $assetWarning = <<<'HTML'
<script {$nonce}>
    console.warn("Actcmscss: The published Actcmscss assets are out of date\n See: https://actcms.work/docs/installation/")
</script>
HTML;
            }
        }
	    ...
	    $devTools = null;
	    $windowActcmscssCheck = null;
	    $windowAlpineCheck = null;
        if (config('app.debug')) {
	        $devTools = 'window.actcmscss.devTools(true);';

	        $windowactcmscssCheck = <<<'HTML'
if (window.actcmscss) {
	    console.warn('Actcmscss: It looks like Actcmscss\'s @actcmscssScripts JavaScript assets have already been loaded. Make sure you aren\'t loading them twice.')
	}
HTML;

	        $windowAlpineCheck = <<<'HTML'
/* Make sure Actcmscss loads first. */
	if (window.Alpine) {
	    /* Defer showing the warning so it doesn't get buried under downstream errors. */
	    document.addEventListener("DOMContentLoaded", function () {
	        setTimeout(function() {
	            console.warn("Actcmscss: It looks like AlpineJS has already been loaded. Make sure actcmscss\'s scripts are loaded before Alpine.\\n\\n Reference docs for more info: http://laravel-actcmscss.com/docs/alpine-js")
	        })
	    });
	}

	/* Make Alpine wait until Actcmscss is finished rendering to do its thing. */
HTML;
...
<script data-turbo-eval="false" data-turbolinks-eval="false"{$nonce}>
    {$windowActcmscssCheck}

    window.actcmscss = new Actcmscss({$jsonEncodedOptions});
    {$devTools}
    window.actcmscss = window.actcmscss;
    window.actcmscss_app_url = '{$appUrl}';
    window.actcmscss_token = {$jsActcmscssToken};

	{$windowAlpineCheck}
    window.deferLoadingAlpine = function (callback) {
        window.addEventListener('actcmscss:load', function () {
            callback();
        });
    };

    document.addEventListener("DOMContentLoaded", function () {
        window.actcmscss.start();
    });
</script>
HTML;
    }
    ...
    public function isActcmscssRequest()
    {
        return $this->isProbablyActcmscssRequest();
    }

    public function isDefinitelyActcmscssRequest()
    {
        $route = request()->route();

        if (! $route) return false;

        return $route->named('actcmscss.message');
    }

    public function isProbablyActcmscssRequest()
    {
        if (static::$isActcmscssRequestTestingOverride) return true;

        return request()->hasHeader('X-Actcmscss');
    }

    public function originalUrl()
    {
        if ($this->isDefinitelyActcmscssRequest()) {
